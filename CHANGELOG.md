...
- 제출 진입점을 `distributional_router`로 바꿨습니다. 공개 Dev 점수는
  ...
  모델을 쓰지 않습니다.
- 표기만 다른 프롬프트는 줄바꿈·줄 끝 공백·Unicode를 맞춘 뒤 같은
  특징으로 보고, 내용이 127종류 이하인 묶음은 Light 아래쪽 비용과
  계열 상단 보정으로만 올립니다. 128종류 이상 경로는 그대로입니다.
  같은 q95 보정을 그 경로에 넣으면 공개 가중 점수가 `0.011761`
  떨어져서 채택하지 않았습니다.
