...
"""E1F — frozen chuf-v1 on exact public costs.

Writes ``build/compare-e1f-cost-conditioned-frontier/`` only. Does not
touch E1–E1E reports, runtime artifacts, or predicted-cost Phase 2.
"""
...
OUT = ROOT / "build" / "compare-e1f-cost-conditioned-frontier"
...
def print_summary(report: Mapping[str, Any]) -> None:
    ...
    print(
        f"E1F public pool n={identity['n_episodes']} "
    ...
    for seed in report["fold_seeds"]:
        ...
        print(f"  ax31_identical={block['ax31_identical_to_baseline']}", flush=True)
        ...
        edges = [
            (row["fold"], row["c_family"], row["c_cell"], row["bin_edges"], row["uk_global"])
            for row in block["fold_frontiers"]
        ]
        print(f"  fold_c_edges_uk={edges}", flush=True)
    ...
    print(
        ...
        f"ax31_identity={gate['ax31_identity_failures']}",
        flush=True,
    )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    print("fitting chuf-v1 (exact public costs; no Phase 2)", flush=True)
