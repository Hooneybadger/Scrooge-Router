...
"""CHUF TV-ball confirmation runner.

Requires a sealed protocol and its canonical SHA. Writes only
``build/confirm-chuf-tvball/``. Refuses to overwrite or to touch E1F
reports. Predicted-cost Phase 2 and runtime export are out of scope.
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    refuse_e1f_output_path(args.output)
    refuse_e1f_output_path(args.audit_output)
    if args.output.exists() or args.audit_output.exists():
        raise RuntimeError(
            "confirmation output already exists; refuse overwrite"
        )
