...
"""E8 Premium residual-parent grid runner.

Requires the sealed e8-premium-residual-grid.v1 protocol. Writes only
``build/run-e8-premium-residual-grid/`` and refuses to overwrite. A pass
...
def print_summary(report) -> None:
    ...
    pin = gate["pin"]
    print(
        f"E8 pin matched={pin['matched']} fidelity={pin['replica_fidelity']} "
    ...
    for arm in CANDIDATE_ARMS:
        block = gate["arms"][arm]
        print(
            f"arm {arm} multiplier={block['multiplier']:.2f} "
            f"train_delta={block['train_delta']:+.8f} "
            f"dev_delta={block['dev_delta']:+.8f} "
            f"fast_bal_identical={block['fast_balanced_identical']} "
            f"residual_ok={block['residual_ok']} "
            f"residual_improved={block['residual_improved']} "
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.output.exists() or args.audit_output.exists():
        raise RuntimeError(
            "e8 premium-residual-grid output already exists; refuse overwrite"
        )
