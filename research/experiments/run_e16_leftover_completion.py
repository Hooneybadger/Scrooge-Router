...
"""E16 leftover-completion runner."""
...
def print_summary(report) -> None:
    ...
    pin = gate["pin"]
    print(
        f"E16 pin matched={pin['matched']} fidelity={pin['replica_fidelity']} "
    ...
    for arm in CANDIDATE_ARMS:
        ...
        print(
            ...
            f"unify={knobs['unify_premium']} residual={knobs['residual_multiplier']} "
            f"word={knobs['leftover_fast_mult']} e11={knobs['e11_cap']} "
            f"e10={knobs['e10_threshold']} "
        ...
        if block["fast_view_failures"]:
            shown = block["fast_view_failures"][:8]
            for row in shown:
                print(
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.output.exists() or args.audit_output.exists():
        raise RuntimeError("e16 output already exists; refuse overwrite")
