...
"""CHUF frozen-runtime fidelity runner.

Requires a sealed protocol and its canonical SHA. Writes only
``build/frozen-runtime-fidelity/``. Refuses overwrite and will not
touch E1F, confirmation, or Phase 2 reports. Runtime export is out
of scope.
"""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.output.exists() or args.audit_output.exists():
        raise RuntimeError("fidelity output already exists; refuse overwrite")
