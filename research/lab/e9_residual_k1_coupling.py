...
"""E9 — residual K1 / brake coupling on the frozen serving path.

E7/E8 closed the Premium parent multiplier grid. Raising that
multiplier freed brake budget and bought more residual K1. This
protocol tests the two already-shipped residual-K1 levers: add
``other`` to the Premium denylist, and reprice residual K1 increment
with the shipped family-guard 2.5. Parent guard, when used, is the
same shipped 2.5. No new constant.
"""
...
EXPERIMENT = "e9-residual-k1-coupling-v1"
REPORT_TYPE = "scrooge-e9-residual-k1-coupling-v1"
SCHEMA_VERSION = 1
...
AUDIT_RELATIVE = "build/run-e9-residual-k1-coupling/episode-audit.json"
REPORT_RELATIVE = "build/run-e9-residual-k1-coupling/report.json"
...
ARM_KNOBS: Mapping[str, ArmKnobs] = {
    BASELINE_ARM: ArmKnobs(False, False, False),
    "residual-k1-denylist": ArmKnobs(False, True, False),
    "residual-k1-guard": ArmKnobs(False, False, True),
    "parent-and-k1-denylist": ArmKnobs(True, True, False),
    "parent-and-k1-guard": ArmKnobs(True, False, True),
}
...
def verify_protocol(
    ...
    *,
    ...
    for arm in CANDIDATE_ARMS:
        ...
        if (
            bool(row["guard_parent"]) != expected.guard_parent
            or bool(row["denylist_other"]) != expected.denylist_other
            or bool(row["guard_brake_k1"]) != expected.guard_brake_k1
        ):
            raise ProtocolError(f"sealed knobs for {arm} drifted")
...
def allocate_arm(
    ...
    knobs = arm_knobs(arm)
    extra = (RESIDUAL_FAMILY,) if knobs.denylist_other else ()
    return replica.allocate_all(
        ...
        guard_brake_k1=knobs.guard_brake_k1,
    )
...
def assemble(
    ...
    *,
    ...
    if output.exists() or audit_output.exists():
        raise ProtocolError("e9 residual-k1-coupling output exists; refuse overwrite")
