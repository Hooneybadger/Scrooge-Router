...
"""E11 — residual AX31/K1 upgrade-fraction cap.

After the shipped Premium allocator, residual rows may keep at most a
pre-registered fraction off Light. The grid is 1/2, 2/3, 4/5 — round
fractions, not a fit to 4.38. Fast/Balanced stay shipped.
"""
...
EXPERIMENT = "e11-residual-upgrade-cap-v1"
REPORT_TYPE = "scrooge-e11-residual-upgrade-cap-v1"
SCHEMA_VERSION = 1
...
ARM_CAPS: Mapping[str, float] = {
    "residual-upgrade-0.80": 0.80,
    "residual-upgrade-0.67": 0.67,
    "residual-upgrade-0.50": 0.50,
}
ARMS: Tuple[str, ...] = (BASELINE_ARM,) + CANDIDATE_ARMS
AUDIT_RELATIVE = "build/run-e11-residual-upgrade-cap/episode-audit.json"
REPORT_RELATIVE = "build/run-e11-residual-upgrade-cap/report.json"
...
def verify_protocol(
    ...
    *,
    ...
    sealed = protocol["arms"]["upgrade_caps"]
    if set(sealed) != set(CANDIDATE_ARMS):
        raise ProtocolError("sealed upgrade cap keys drifted")
    for arm, expected in ARM_CAPS.items():
        if abs(float(sealed[arm]) - float(expected)) > 1e-12:
            raise ProtocolError(f"sealed upgrade cap for {arm} drifted")
...
def arm_upgrade_cap(arm: str) -> Optional[float]:
    if arm == BASELINE_ARM:
        return None
    if arm not in ARM_CAPS:
        raise ProtocolError(f"unknown arm {arm}")
    return float(ARM_CAPS[arm])
...
def allocate_arm(
    ...
    selections = replica.allocate_all(
        split, indexes, caps=replica.shipped_caps, guard_parent=False
    )
    cap = arm_upgrade_cap(arm)
    if cap is None:
        return selections
    families = [split.families[index] for index in indexes]
    quality = [split.premium_quality[index] for index in indexes]
    digests = [split.digests[index] for index in indexes]
    selections["premium"] = demote_residual_upgrades(
        families, selections["premium"], quality, digests, cap
    )
    return selections
...
def evaluate_arm_on_split(
    ...
    return {
        ...
        "stress": stress,
        "upgrade_cap": None if cap is None else json_float(cap),
    }
...
def assemble(
    ...
    *,
    ...
    if output.exists() or audit_output.exists():
        raise ProtocolError("e11 residual-upgrade-cap output exists; refuse overwrite")
    ...
    for arm in CANDIDATE_ARMS:
        ...
        gate["arms"][arm] = {
            ...
            "upgrade_cap": json_float(ARM_CAPS[arm]),
        }
        if passed:
            ranked.append(arm)
    if ranked:
