...
"""E14 — Fast predicted-cap when the top two families dominate the batch.

E13 tightens Fast only when one family is at least 0.75. Two-family
50/50 batches never bind that rule. The statistic here is the top-two
family mass, not a named-family multiplier. Public official top-2 is
about 0.34, so 0.25 would bind the official mix and is not a candidate.
The grid is 0.50 / 0.75, the E13 points that stay above that public
mix. Caps are the E12 values that cleared Fast family views: 1.07 and
1.05. 1.08 is not a candidate. Runaway stays 0.165. Pair views are
every eligible family pair, not the red-team leftover set.
"""
...
EXPERIMENT = "e14-top2-fast-cap-v1"
REPORT_TYPE = "scrooge-e14-top2-fast-cap-v1"
SCHEMA_VERSION = 1
...
ARM_KNOBS: Mapping[str, ArmKnobs] = {
    ...
    "cond-top2-1.07-0.75": ArmKnobs(0.75, 1.07),
    "cond-top2-1.07-0.50": ArmKnobs(0.50, 1.07),
    "cond-top2-1.05-0.75": ArmKnobs(0.75, 1.05),
    "cond-top2-1.05-0.50": ArmKnobs(0.50, 1.05),
}
ARMS: Tuple[str, ...] = (BASELINE_ARM,) + CANDIDATE_ARMS
AUDIT_RELATIVE = "build/run-e14-top2-fast-cap/episode-audit.json"
REPORT_RELATIVE = "build/run-e14-top2-fast-cap/report.json"
...
def verify_protocol(
    ...
    *,
    ...
    for arm in CANDIDATE_ARMS:
        ...
        if abs(float(row["threshold"]) - 0.25) <= 1e-15:
            raise ProtocolError("0.25 binds the official top-2 mix")
        if abs(float(row["fast_cap"]) - 1.08) <= 1e-15:
            raise ProtocolError("1.08 is not a candidate")
...
def shipped_fast_cap(families: Sequence[str]) -> float:
    if budget_brake_router.fast_family_composition_guard(families):
        return float(budget_brake_router.CONDITIONAL_FAST_CAP)
...
def arm_caps(
    ...
    caps["fast"] = shipped_fast_cap(families)
    knobs = arm_knobs(arm)
    if knobs.threshold is None or knobs.fast_cap is None:
        return caps
    if top2_family_fraction(families) + 1e-15 >= float(knobs.threshold):
...
def allocate_arm(
    ...
    return replica.allocate_all(
        ...
        guard_parent=active,
        denylist_extra=("other",) if active else (),
    )
...
def evaluate_arm_on_split(
    ...
    for name, indexes in views.items():
        ...
        families = [split.families[index] for index in indexes]
        max_family = max_family_fraction(families)
        top2 = top2_family_fraction(families)
        caps = arm_caps(replica, families, arm)
        stress[name] = {
            "caps": dict(caps),
            "max_family_fraction": json_float(max_family),
            "top2_family_fraction": json_float(top2),
        }
        for tier in TIERS:
    ...
    return {
        ...
        "stress": stress,
        "top2_family_fraction": json_float(top2_family_fraction(split.families)),
    }
...
def assemble(
    ...
    *,
    ...
    if output.exists() or audit_output.exists():
        raise ProtocolError("e14 top2-fast-cap output exists; refuse overwrite")
    ...
    for label, split in (("train", train), ("dev", dev)):
        top2 = top2_family_fraction(split.families)
        if top2 + 1e-15 >= LOWEST_THRESHOLD:
            raise ProtocolError(
                f"official {label} top-2 {top2:.6f} reached the lowest threshold"
            )
