...
행은 건너뛰고 계속 스캔하며, 개수 상한 48에서 멈춥니다. 미리 정한 계열과
폭주 가드에 걸린 증분은 후보에서 제외합니다.

Premium의 폭주 가드는 두 값 중 작은 쪽입니다. 하나는 Train 전체 배치의
2%로 굳은 상수 `runaway_absolute` 0.1715이고, 다른 하나는 받은 배치의
예측 light에 `runaway_share` 0.06을 곱한 값입니다. 상수만 쓰면 짧거나 싼
배치에서 한 문항이 그 배치 자신의 예산에서 큰 몫을 가져갑니다. 배치 예측
light가 2.8588 이상이면 상수 쪽이 작아 선택이 바뀌지 않고, 그 아래에서만
배치 기준이 조입니다.
