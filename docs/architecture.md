...
```
family_guard_router      계열 가드      ← 제출되는 라우터
      │
...
`family_guard_router`가 제출되는 라우터입니다. 프롬프트 텍스트만으로 문항을
몇 가지 묶음으로 나누고, 예측이 자주 빗나가는 묶음의 **회계상** 비용을
올린 뒤 2층의 배분기를 돌립니다.
...
Premium 등급은 가드를 쓰지 않고 2층에 그대로 넘깁니다. Premium은 한도가
넉넉해서 예측 오차가 한도를 위협하지 않습니다.
